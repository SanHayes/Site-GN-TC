...
function getconf($field='')
{
    ...
    foreach($res as $key => $item){
        if($key == 'sys_app_url' && !empty($res[$key])){
            $res[$key] = (isHTTPS() ? 'https://' : 'http://').$_SERVER['HTTP_HOST'].'/99/app';
        }
    }
    return $res;
}
		...
		$html .= '<div style="display:none" class="oid_list oid_'.$vo['oid'].'">
	                  <div class="vo_son" style="margin-left: '.$margin_left.'px;"><p>|——'.$type.'级代理</p></div>
	                    <div class="div_my_son">
	                      <ul class="my_sons">
	                        <li>代理名：'.$vo['username'].' 餘額：'.$vo['usermoney'].'</li>
	                        <li>手機：'.$vo['utel'].' <a href="/admin/user/userlist.html?uid='.$vo['uid'].'"><button class="btn btn-primary btn-xs">詳情</button></a></li>
	                      </ul>
...
function cash_oid($uid) {
	...
	cache('cash_oid'.$uid,'<td> <a title="點選檢視" href="/admin/user/userlist.html?uid='.$uid.'"> '.$uid.' </a> </td><td '.$class.'>'.$minprice.'</td>',20);
	return '<td> <a title="點選檢視" href="/admin/user/userlist.html?uid='.$uid.'"> '.$uid.' </a> </td><td '.$class.'>'.$minprice.'</td>';
}
