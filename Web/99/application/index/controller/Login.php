...
class Login extends Controller
{
    ...
    */
    public function login()
    {
        ...
        if (input('post.')) {
            ...
            if (!$result) {
                echo "<script>alert('登入失敗,使用者名稱不存在!')</script>";
                return $this->fetch();
            }
            //只有代理商和普通客户可以前端登录
            if (!in_array($result['otype'], array(0, 101))) {
                echo "<script>alert('您無權登入!')</script>";
                return $this->fetch();
            }

            //帐号冻结
            if ($result['ustatus'] == 1) {
                echo "<script>alert('登入失敗,您的賬戶暫時被凍結!')</script>";
                return $this->fetch();
            }
    ...
    public function register()
    {
        ...
        if(input('post.')){
            ...
            if(!isset($data['username']) || empty($data['username'])){
                return WPreturn('請輸入使用者名稱！',-1);
            }
            if(!isset($data['upwd']) || empty($data['upwd'])){
                return WPreturn('請輸入密碼！',-1);
            }
            if(!isset($data['upwd2']) || empty($data['upwd2'])){
                return WPreturn('請再次輸入密碼！',-1);
            }
            ...
            if ($uid) {
                ...
                if($data['oid'] && $reward['reg_money']>0) {
                    ...
                    if($flag) {
                        ...
                        //资金日志
                        set_price_log($data['oid'],1,$reward['reg_money'],'邀請獎勵','邀請註冊獎勵',$bpid,$usermoney);
                    }
                }
				...
				if($this->conf['sys_reg_caijin']>0){
					//资金日志
					set_price_log($uid,1,$this->conf['sys_reg_caijin'],'彩金','註冊送彩金',0,0);
					cache('caijin', '註冊送彩金'.$this->conf['sys_reg_caijin'].'元', 60);
				}

                return WPreturn('註冊成功，已自動登入!',1);
            }else{
                return WPreturn('註冊失敗,請重試!',-1);
            }
    ...
    */
	...
	protected function orderauto($oid, $price)
	{
        ...
        trace('login - orderauto 訂單的執行', 'sql');
        ...
        if (($order['ostyle'] == 1 && $order['buyprice'] < $price) || ($order['ostyle'] == 0 && $order['buyprice'] > $price)) {
            ...
            $up_price_log['title'] = "結單";
            $up_price_log['content'] = '訂單輸結算';
