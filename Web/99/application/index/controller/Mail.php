...
class Mail extends Controller
{
    function sendMail()
    {
        $email = input('email');
        if(!$email){
            return json(['code'=>-102,'data'=>[],'msg'=>'請輸入郵箱']);
        }else{
        ...
        if($this->send_mail($email,'註冊驗證碼',$content)){
            $data['number'] = $email;
            $data['code'] = $code;
            $data['type'] = 'mail';
            $data['time'] = time();
            if(!$user_code){
                if(Db::name('usercode')->insertGetId($data)){
                    ...
                    return json(['code'=>-62,'data'=>null,'msg'=>'操作失敗2！']);
                }
            }else{
                if(Db::name('usercode')->where(['number'=>$email])->update($data)){
                    return json(['code'=>200,'data'=>$email,'msg'=>'success']);
                }else{
                    return json(['code'=>-62,'data'=>null,'msg'=>'操作失敗3！']);
                }
            }
        }else{
            return json(['code'=>-62,'data'=>null,'msg'=>'操作失敗1！']);
        }
    }
