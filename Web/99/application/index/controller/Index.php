...
class Index extends Base
{
    ...
    public function index()
    {
		...
		if($this->conf['sys_homepage']==1){
			$this->redirect(Url('/index/index/home'));
		}
        //获取产品信息
        ...
        foreach ($pro as $k => $v) {
            $pro[$k]['isopen'] = ChickIsOpen($v['pid']);
	...
	public function ajax_order(){
		...
		//$price_arr = array(100,200,300,400,500,600,700,800,500,500,1000,1000買跌,5000,5000,10000,20000);
		//$price_arr_length = count($price_arr);
		$type_arr = array('買漲','買跌');
    ...
    public  function home() {
		...
		// 	$this->redirect('index/index/index?token='.$this->token);
		// }
    ...
    public  function  inquiries() {
        $uid = $_SESSION['uid'];
        if(!$uid){
            $this->redirect(Url(url('index/login/login')));
        }
    ...
    public  function card_reset() {
        ...
        if($res){
            return WPreturn('銀行卡解綁成功', 1);
        }else{
            return WPreturn('銀行卡解綁失敗');
        }
    }
    ...
    public function modityepwd()
    {
        ...
        if (request()->isPost()) {
            ...
            if ($flag) {
                return WPreturn('提現密碼修改成功', 1);
            } else {
                return WPreturn('提現密碼修改失敗');
            }
        } else {
    ...
    public function withdraw() {
        ...
        if(input('post.')){
            ...
            if(!$bankinfo){
                $this->redirect(url('index/index/bankcard'));
            }else{
    ...
    public  function  add_bank() {
        ...
        if (input('post.')) {
            $data = input('post.');
			if(!$data['bankno']){
				echo '<script>alert("請設定提現銀行卡");history.back();</script>';
                exit;
			}
            if(!$uid){
                $this->redirect(url('index/login/login'));
            ...
            if($user['nickname'] && $user['nickname']!=$data['accntnm']){
                echo '<script>alert("持卡人姓名與註冊姓名不一致，請聯絡客服處理");history.back();</script>';
                exit;
            }
            $arr = [
         ...
         header("Location:".Url('/index/index/bankcard'));
         exit;
        }
        $user = db('userinfo')->field('nickname')->where(['uid'=>$uid])->find();
