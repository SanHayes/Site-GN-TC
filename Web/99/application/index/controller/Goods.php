...
class Goods extends Base
{
	...
	public function goods()
	{
		...
		$pay_choose_arr_money = $pay_choose_arr;
		foreach ($pay_choose_arr_money as $key=>$val){
		    if($val == '全部金额'){
		        $pay_choose_arr_money[$key] = $this->user['usermoney'];
		    }
		}
		...
		$isopen = !ChickIsOpen($pid);
 		...
 		$this->assign('pay_choose_arr_money',$pay_choose_arr_money);
 		$this->assign('isopen',$isopen);
 		$this->assign('userinfo',$this->user);
 		$risk = db('risk')->find();
 		//$risk['base'] = $risk['min_yk']<10 && 10<$risk['max_yk'] ? 10 : $risk['min_yk'];
