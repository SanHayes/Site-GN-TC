...
class Index extends Base
{
    ...
    {
        if(!input('token')){
            //$this->redirect('home/login/login?token='.$this->token);
        }
		if($this->conf['sys_homepage']==1){
			$this->redirect('home/index/home');
		}
        //获取产品信息
        $pro = db('productdata')->field('pid,Name ptitle,Price,UpdateTime,Low,High')->where('isdelete',0)->order('sort')->select();
        $this->assign('pro',$pro);
    ...
    {
    	//获取产品信息
        $pro = Db::name('productdata')->field('pid,Name ptitle,Price,UpdateTime,Low,High')->where('isdelete',0)->select();
        $newpro = array();
        foreach ($pro as $k => $v) {
        	$newpro[$v['pid']] = $pro[$k];
    ...
    public  function ajaxdata() {
        $product = db('productdata')->cache(30)->field("pid,name,price,isdelete,high,low")->where(array('isdelete' => 0))->select();
    ...
    public  function home() {
        ...
        foreach ($product as $key=>$item) {
            ...
            $item['isopen'] = ChickIsOpen($item['pid']);
    ...
    public function paysubmit(){
        ...
        if($bpprice<$this->conf['userpay_min']){
            return ['status'=>false,'msg'=>'最小充值金額'.$this->conf['userpay_min'].'起'];
        }elseif($bpprice>$this->conf['userpay_max']){
            return ['status'=>false,'msg'=>'最大充值金額'.$this->conf['userpay_max'].'元'];
        }

        // 充值方式
        $payment = db('payment')->where(['is_use'=>1,'isdelete'=>0,'id'=>$pay_type])->find();
        if(!$payment){
            return ['status'=>false,'msg'=>'請選擇充值方式'];
        }
        $user = db('userinfo')->field('usermoney')->where(['uid'=>$uid])->find();
        $bpbalance = $user['usermoney']+$bpprice;
        $insert = [
            'uid' => $uid,
            'bpprice' => $bpprice,
            'pay_type' => trim($payment['pay_conf'], 'name:'),
            'bptype' => 1,
            'bptime' => time(),
        ...
        if($bid){
            return ['status'=>true,'msg'=>'充值稽覈中'];
        }else{
            return ['status'=>false,'msg'=>'充值失敗'];
        }
    }
    ...
    public  function  inquiries() {
		...
		foreach($orders as  $k=>$val) {
			...
			$orders[$k]['fx'] = ($val['ostyle']==0)?'買漲':'買跌';
    ...
    public  function accountrecord() {
        ...
        $acountrf = db('balance')->where(['uid'=>$uid,'isshow'=>1])->whereIn('bptype', [1,2,3,4])->order('bpid desc')->select();
        $dealhis = [];
        $types = [1=>'充值',2=>'手動加款',3=>'正在充值',4=>'取消','5'=>'提現','6'=>'手動扣款',7=>'邀請獎勵'];
        foreach($acountrf  as $key=>$val) {
            $dealhis[$key]['utime'] = date('Y-m-d H:i:s',$val['bptime']);
            $dealhis[$key]['cltime'] = date('Y-m-d H:i:s',$val['bptime']);
            if (!is_null($val['cltime'])){
                $dealhis[$key]['cltime'] = date('Y-m-d H:i:s',$val['cltime']);
            }
            ...
            if ($val['isverified'] === 0) {
                $dealhis[$key]['is_verify'] = '稽覈中';
            } elseif ($val['isverified'] === 1) {
                $dealhis[$key]['is_verify'] = '稽覈通過';
            } elseif ($val['isverified'] === 2) {
                $dealhis[$key]['is_verify'] = '拒絕';
            } else {
                $dealhis[$key]['is_verify'] = '待稽覈';
            }
    ...
    public function withdrawrecord(){
        ...
        $acountrf = db('balance')->where(['uid'=>$uid])->whereIn('bptype', [5])->order('bpid desc')->select();
        $dealhis = [];
        $types = [1=>'充值',2=>'手動加款',3=>'正在充值',4=>'取消','5'=>'提現','6'=>'手動扣款',7=>'邀請獎勵'];
        foreach($acountrf  as $key=>$val) {
            $dealhis[$key]['utime'] = date('Y-m-d H:i:s',$val['bptime']);
            $dealhis[$key]['ctime'] = date('Y-m-d H:i:s',$val['bptime']);
            if (!is_null($val['cltime'])){
                $dealhis[$key]['ctime'] = date('Y-m-d H:i:s',$val['cltime']);
            }
            $dealhis[$key]['reg_par'] = $val['reg_par'];
            $dealhis[$key]['realprice'] = $val['realprice'];
            $dealhis[$key]['typedesc'] =$types[$val['bptype']];
    ...
    public  function bankcard() {
        ...
        $this->assign('card_delete_count', db('config')->where('name', 'card_reset')->value('value'));
    ...
    public function card_reset() {
        db('bankcard')->where(['uid'=>$this->user['uid']])->delete();
        db('userinfo')->where(['uid'=>$this->user['uid']])->setInc('card_delete_count');
        return WPreturn('解除成功', 1);
    }
    ...
    public  function  moditypwd() {
        ...
        if(request()->isPost()){
            ...
            if($user['upwd'] != $loginpwd){
                return WPreturn('登入密碼不正確');
            }
            if($post['password'] != $post['cpassword']){
                return WPreturn('提款密碼與確認密碼不一致');
            }
            $upwd = $post['password'];
            $flag = Db::name('userinfo')->where('uid',$this->uid)->update(['upwd'=>$upwd]);
            if($flag){
                return WPreturn('登入密碼修改成功', 1);
            }else{
                return WPreturn('登入密碼修改失敗');
            }
        }else{
            return $this->fetch();
        }
    ...
    public function withdraw() {
        ...
        if(input('post.')){
            ...
            if($data){
                if(!$data['price']){
                    return WPreturn('請輸入提現金額');
                }
				if(empty($data['bankcardno'])){
					 return WPreturn('請設定提現銀行卡');
				}
                //验证申请金额
                $user = $this->user;
                if($user['ustatus'] == 1){
        			return WPreturn('您的賬戶已被凍結,請聯絡線上客服！',-1);
        		}
        		
        		if($user['ustatus'] == 2){
        			return WPreturn('您的賬戶已被限制交易,請聯絡線上客服！',-1);
        		}

                $conf = $this->conf;
                if($conf['is_cash'] != 1){
                    return WPreturn('抱歉！暫時無法出金');
                }
                if($conf['cash_min'] > $data['price']){
                    return WPreturn('單筆最低提現金額為：'.$conf['cash_min']);
                }
                if($conf['cash_max'] < $data['price']){
                  return    WPreturn('單筆最高提現金額為：'.$conf['cash_max']);
                }

                $epwd = $data['passwd'];
                if($user['epwd'] != $epwd){
                    return    WPreturn('提款密碼不正確');
                }
                ...
                if( ($this->user['usermoney'] - $data['price']) < 0){
                    return WPreturn('最多提現金額為'.$this->user['usermoney'].'元');
                }
              ...
              //  $mybank = db('bankcard')->where('uid',$uid)->find();
                ...
                return WPreturn('暫不支付此提現型別！');
            }
        }else{

            $bankinfo = db('bankcard')->where(['uid'=>$uid])->order('id desc')->find();
            if(!$bankinfo){
                $this->redirect(url('home/index/bankcard'));
            }else{
    ...
    public function add_bank() {
        ...
        if (input('post.')) {
            ...
            if (!$user['nickname'] && $data['accntnm']) {
                db('userinfo')->update([
                    'uid' => $uid,
                    'nickname' => $data['accntnm'],
                ]);
            }
            if($user['nickname'] && $user['nickname']!=$data['accntnm']){
                return [
                    'data' => '持卡人姓名與註冊姓名不一致，請聯絡客服處理',
                    'type' => -1,
                ];
            }
            $arr = [
                'accntno' => $data['bankno'], //卡号
                'scard' => $data['scard'], //身份证号码
    ...
    public function forgot(){
        ...
        if(request()->isPost()){
            ...
            if($flag){
                return WPreturn('找回提款密碼成功', 1);
            }else{
                return WPreturn('找回提款密碼失敗');
            }
        }else{
            return $this->fetch();
        }
	...
	public function investrecord() {
        ...
        $list = db('userinvest')->where(['uid'=>$uid])->order('id desc')->select();
        $total = db('userinvest')->where(['uid'=>$uid])->count();
	...
	public function investlist() {
        ...
        $list['list'] = db('invest')->where(['state'=>1])->order('pid')->select();
		$sums = [];
		foreach($list['list'] as $k=>$ls){
		    $list['list'][$k]['days'] = $ls['days'].'天';
			$sums[] = $ls['days'];
		}
		if($sums) {
            $list['sums'] = implode('，', $sums);
        }
        $list['total'] = db('invest')->where(['state'=>1])->count();
        return $list;
	}
	...
	public function doinvest() {
		...
		if($money<$item['min']){
			return WPreturn('投資金額必須大於最小起投金額');
		}
		if($money>$this->user['usermoney']) {
			return WPreturn('投資失敗，您的可用餘額不足');
		}
		...
		if($flag){
			...
			if($id){
				//资金日志
				if($this->conf['sys_yue_benjin']==2){
					set_price_log($uid,2,$money,'利息寶','投資',$id,$this->user['usermoney']);
				}
				return WPreturn('投資成功', 1);
			}else{
				return WPreturn('投資失敗');
			}
		}else{
			return WPreturn('投資失敗，餘額不足');
		}
	}
