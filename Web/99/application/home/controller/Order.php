	...
	public function addorder()
	{
		...
		*/
		...
		if($conf['sys_buy_once'] == 1) {
			...
			if($iorder) return WPreturn('您有待結算的持倉，請勿重複下單！',-1);
		}
...
// 		}
		...
		try{
			...
			if($ids){
				...
				set_price_log($data['uid'],2,$u_fee,'下單','下單成功',$ids,$nowmoney);
				...
				if($editmoney){
					...
					if($reward['invest_percent']>0 && $user['oid']>0) {
						...
						if($flag) {
							...
							//资金日志
							set_price_log($user['oid'],1,$invest_money,'邀請獎勵','下級交易分成',$bpid,$usermoney);
						}
					}
	...
	function pcorder() {
		...
		if(!$order){
			return ['type'=>0,'data'=>'訂單不存在','price'=>0];
		}elseif($order['ostaus']!=0){
			return ['type'=>0,'data'=>'訂單已結算','price'=>0];
		}

		$sys_pingcang = getconf('sys_pingcang');
		if($sys_pingcang==0) {
			return ['type'=>0,'data'=>'手動平倉未開啟','price'=>0];
		}
		...
		if(!$flag){
			return ['type'=>0,'data'=>'訂單結算失敗','price'=>0];
		}
		//平仓增加用户金额
	...
	public function set_order_log($v,$addprice)
	{
		$o_log['uid'] = $v['uid'];
       	...
       	//资金日志
       	set_price_log($v['uid'],1,$addprice,'結單','訂單到期獲利結算',$v['oid'],$o_log['user_money']);
	}
	...
	public function hold()
	{
		...
		$uid = $_SESSION['uid'];		
		$hold = Db::name('order')->where(array('uid'=>$uid,'ostaus'=>0))->order('oid desc')->select();
		foreach ($hold as $key => $item) {
            $hold[$key]['buytime'] = date('Y-m-d H:i:s', $item['buytime']);
            $hold[$key]['pro'] = GetProData($item['pid'], 'pi.ptitle,pi.procode,pi.protime,pi.propoint,pi.proscale,pd.*');
            $hold[$key]['time'] = $item['selltime'] - time();
        }
	...
	{
		$page = ceil(input('get.offset')/input('get.limit')) + 1;
        $uid = $this->uid;
        $hold = Db::name('order')->where(array('uid' => $uid, 'ostaus' => 1))->order('oid desc')->paginate(10,false,['type' => 'Bootstrap','var_page' => 'page','page' => $page]);
        $hold->getCollection()->each(function ($item, $index) use ($hold) {
            $item['ostyle'] = $item['ostyle'] == 0 ? '買漲' : '買跌';
